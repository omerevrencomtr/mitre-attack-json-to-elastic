...
def get_mitre_attack_technique_groups(tactic_name, technique_name, technique_id, identifier, mitre_attack_relationships,
                                      mitre_attack_threat_groups):
    groups = []
...
def get_mitre_attack_technique_software(tactic_name, technique_name, technique_id, identifier,
                                        mitre_attack_relationships, mitre_attack_attack_software):
    software = []
    for relationship in mitre_attack_relationships:
        if (relationship.get('target_ref') == identifier):
    ...
    for technique in mitre_attack_techniques:
        ...
        if kill_chain_phases:
            for kill_chain_phase in kill_chain_phases:
                ...
                data_sources = get_mitre_attack_technique_datasources(tactic_name, technique_name, technique_id,
                                                                      technique.get('x_mitre_data_sources'))
                platforms = get_mitre_attack_technique_platforms_and_permissions(tactic_name, technique_name,
                                                                                 technique_id, technique)
                groups = get_mitre_attack_technique_groups(tactic_name, technique_name, technique_id, item_id,
                                                           mitre_attack_relationships, mitre_attack_threat_groups)
                software = get_mitre_attack_technique_software(tactic_name, technique_name, technique_id, item_id,
                                                               mitre_attack_relationships, mitre_attack_attack_software)
...
def main():
    util = utils.utils()
    util.download_and_save_mitre_attack_matrix()
